# This is a rumor detector that uses Logistic Regression to detect.
...
class LogisticRumorDetector:
    def __init__(self, max_iter=1000, random_state=19260817) -> None:
        """
        Initialize the Logistic Rumor Detector.
    ...
    def preprocess_dataframe(df: pd.DataFrame) -> Tuple[pd.Series, pd.Series]:
        ...
        Args:
            df (pd.DataFrame): The raw data containing text and labels.

        Returns:
            Tuple[pd.Series, pd.Series]: The preprocessed text data and labels.
        """
        text_df = df['text'].fillna('')
        labels = df['label']
        text_df = text_df.apply(LogisticRumorDetector.preprocess_text)
        return text_df, labels

    def fit(self, train_data: pd.Series, train_labels: pd.Series) -> None:
        """
        Train the Logistic Rumor Detector.
        Note that this method should be used after the original data has been PREPROCESSED

        Args:
            train_data (pd.Series): The cleaned train data
            train_labels (pd.Series): The labels that used to train the model
        """
        print("正在处理文本特征...")
        self.tfidf_vectorizer.fit(train_data)
        train_tfidf = self.tfidf_vectorizer.transform(train_data)
        print("正在训练模型...")
        self.model.fit(train_tfidf, train_labels)
    ...
    def classify(self, text: str) -> int:
        ...
        prediction = self.model.predict(test_tfidf)
        return prediction[0]
